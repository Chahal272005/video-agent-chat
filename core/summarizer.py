...
def summarize(transcript: str) -> str:
    ...
    map_prompt = ChatPromptTemplate.from_messages(
        [
            ("system", "Summarize this portion of the transcript concisely."),
            ("human", "{text}"),
        ]
    )
...
def generate_title(transcript: str) -> str:
    ...
    title_prompt = ChatPromptTemplate.from_messages(
        [
            ("system", "Based on the meeting transcript, generate a short professional title for the meeting."
             "(max 8 words). Only return the title, do not include any other text."),
            ("human", "{text}"),
        ]
    )
    title_chain = (
